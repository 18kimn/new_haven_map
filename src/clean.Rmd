...
This notebook contains code necessary to clean the data used in the Counter Haven project. Data are read in from `input_data/raw` or downloaded straight from the web, and then either written to `input_data/clean` (if they will be uploaded to Mapbox Studio) or to `src/static/data` (if we want them to be loaded by the client-side at runtime).  
...
This in turn requires using the generic JSON format instead of GeoJSON, because `write_sf` and other GeoJSON handlers can't handle list-columns. But that's okay, we can clean the data up in a tiny JavaScript chunk as shown here.
...
dta <- get_decennial("block", variables = c("P008001", "P008004"),
  ...
  mutate(holc = st_within(centroids, st_transform(holc, st_crs(.))) %>% 
           map(~holc$holc_id[.[1]]) %>% 
  #necessary to turn blocks with no HOLC id to NA and thus get a vector of the right length
           unlist()) %>% 
...
#when we're translating inside the animation, we want the overall center of the image to more or less remain where it is -- 
# we don't want things to suddenly fly towards a point off of the grid for some reason when the transition begins
...
Finally, the translated coordinates are turned into a 32-fps video and a frame highlighting the owners. These are saved in `src/static`. This chunk also takes an unbelievably long time to run, so I'm using `eval=FALSE` on the chunk header. 
...
paste0(here("input_data", "frames"), "/", c(1:80 , 80:1), ".png") %>% 
  map(image_read) %>% 
  image_join() %>% 
  image_write_video(here("src", "static", "grid.mp4"), 
                    framerate = 32)
